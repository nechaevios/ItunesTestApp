...
//  ItunesTestApp
//
//  Created by Nechaev Sergey  on 06.01.2022.
//
...
class DetailAlbumViewController: UIViewController {
    ...
    private func setImage(urlString: String?) {
        ...
        if let url = urlString {
            NetworkManager.shared.fetchData(urlString: url) { [weak self] result in
                switch result {
    ...
    private func fetchSongs(album: Album?) {
        ...
        let urlString = "http://itunes.apple.com/lookup?id=\(idAlbum)&entity=song"
        
        NetworkManager.shared.fetchSong(urlString: urlString) { [weak self] songModel, error in
            if error == nil {
